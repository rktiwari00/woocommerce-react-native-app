  ...
  const renderProductCard = (product) => (
    <TouchableOpacity key={product.id} style={styles.productCard} onPress={() => handleProductPress(product)}>
      <Card style={styles.card}>
        <Card.Cover source={{ uri: product.images?.[0]?.src || 'https://via.placeholder.com/200x200' }} style={styles.productImage} />
        <Card.Content style={styles.cardContent}>
          <Text style={styles.productName} numberOfLines={2}>{product.name}</Text>
          <View style={styles.priceContainer}>
            {product.sale_price && (
              <Text style={styles.regularPrice}>{storeConfig.payment.currencySymbol}{product.regular_price}</Text>
            )}
            <Text style={styles.price}>{storeConfig.payment.currencySymbol}{product.sale_price || product.price}</Text>
          </View>
          <Button mode="contained" onPress={() => handleAddToCart(product)} style={styles.addToCartButton} labelStyle={styles.buttonLabel}>
            Add to Cart
          </Button>
        </Card.Content>
      </Card>
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      <View style={styles.header}>
        <LinearGradient colors={[theme.primary, theme.primaryDark]} style={styles.headerGradient}>
          <View style={styles.headerContent}>
            <Text style={styles.welcomeText}>{storeConfig.content.welcomeMessage}</Text>
      ...
      <View style={styles.bannerContainer}>
        <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false}>
          {banners.map((uri, idx) => (
            <Image key={idx} source={{ uri }} style={styles.bannerImage} resizeMode="cover" />
          ))}
        </ScrollView>
      </View>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity onPress={() => router.push('/(tabs)/categories')}><Text style={styles.seeAllText}>See All</Text></TouchableOpacity>
        </View>
      ...
      {onSaleProducts.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Chip icon="sale" mode="outlined" style={styles.saleChip}>Up to 50% Off</Chip>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
...
const styles = StyleSheet.create({
  ...
  headerGradient: { paddingTop: 50, paddingBottom: theme.spacing.lg, paddingHorizontal: theme.spacing.md },
  headerContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  welcomeText: { fontSize: theme.typography.h3.fontSize, fontWeight: 'bold', color: theme.textLight, flex: 1 },
  cartButton: { position: 'relative', padding: theme.spacing.sm },
  cartBadge: { position: 'absolute', top: 0, right: 0, backgroundColor: theme.error, borderRadius: theme.borderRadius.round, minWidth: 20, height: 20, justifyContent: 'center', alignItems: 'center' },
  cartBadgeText: { color: theme.textLight, fontSize: theme.typography.small.fontSize, fontWeight: 'bold' },
  bannerContainer: { marginHorizontal: theme.spacing.md, marginBottom: theme.spacing.lg, borderRadius: theme.borderRadius.lg, overflow: 'hidden' },
  ...
  price: { fontSize: theme.typography.h4.fontSize, fontWeight: 'bold', color: theme.primary },
